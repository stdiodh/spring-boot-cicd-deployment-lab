...
이번 answer는 09 시퀀스에서 만든 배포 가능한 앱을 같은 순서로 반복 실행하도록 workflow와 script를 연결한 비교 기준입니다.
...
2. `.github/workflows/deploy.yml`에서 job 의존성과 artifact 전달 흐름을 확인합니다.
3. `scripts/deploy.sh`에서 서버 재배포 순서를 확인합니다.
4. `scripts/check-deploy.sh`에서 컨테이너 상태, 로그, HTTP 응답 확인을 확인합니다.
...
`.github/workflows/deploy.yml`에서 build job, deploy job, verify job이 어떤 순서로 연결되는지 확인합니다.
...
`scripts/check-deploy.sh`에서 컨테이너 상태, 애플리케이션 로그, HTTP 응답 확인을 확인합니다.
...
- starter와 비교할 때 job 의존성, artifact 구성, script 실행 위치, verify 기준 순서로 확인합니다.
- 힌트가 필요하면 실패한 단계 이후 작업이 왜 멈춰야 하는지부터 질문합니다.
