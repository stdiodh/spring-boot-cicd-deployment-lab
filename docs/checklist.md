...
- [ ] 실행 기준 브랜치가 `10-answer`인지 확인했습니다.
- [ ] 09 Docker Runtime과 10 CI/CD 계약이 한 브랜치에 누적되어 있음을 확인했습니다.
...
- [ ] Docker Hub에는 tag 대상 40자리 commit SHA와 `deploy-vX.Y.Z` tag가 게시됩니다.
...
- [ ] 실패 시 기존 tag를 이동하지 않고 정상 commit에 새 배포 tag를 만들어 rollback합니다.
...
- [ ] `10-answer`의 commit을 push한 뒤 annotated deploy tag를 만들었습니다.
